$execute if data storage unknown_pack_name:puzzle_game {puzzle_active:1b} if items block ~ ~ ~ container.$(slot) stone[custom_data~{puzzle_piece:1b}] run return 0

$execute if items block ~ ~ ~ container.$(slot) stone[custom_data~{word_puzzle:1b}] run data modify storage unknown_pack_name:temp clicked_word_$(slot) set from block ~ ~ ~ Items[{Slot:$(slot)b}].components."minecraft:custom_data".word_value
...
$execute if score #recipe_filled temp matches 1 unless items block ~ ~ ~ container.$(slot) light_gray_stained_glass_pane[custom_data={gui_filler:1b}] if items block ~ ~ ~ container.$(slot) * unless items block ~ ~ ~ container.$(slot) barrier[custom_data~{pottery_result:1b}] run function unknown_pack_name:active/pottery_wheel/return_item {slot:$(slot)}

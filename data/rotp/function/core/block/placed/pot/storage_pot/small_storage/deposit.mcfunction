execute unless data entity @p[distance=..10,tag=current_player] SelectedItem.id run return 0
execute store result storage rotp:temp storage_id int 1 run scoreboard players get @s storage_pot_id

data modify storage rotp:temp player_item set from entity @p[distance=..10,tag=current_player] SelectedItem

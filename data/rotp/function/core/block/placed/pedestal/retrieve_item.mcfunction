...
execute at @a[tag=pedestal_interact,limit=1,sort=nearest] run summon item ~ ~ ~ {Item:{id:"stone",count:1},PickupDelay:0,Motion:[0.0,0.1,0.0],Tags:[new_item]}
execute at @a[tag=pedestal_interact,limit=1,sort=nearest] as @e[type=item,distance=..1,limit=1,sort=nearest,tag=new_item] run data modify entity @s Item set from entity @e[tag=pedestal_interaction,limit=1,sort=nearest,tag=being_retreived] data.stored_item

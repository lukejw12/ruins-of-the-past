...
$execute if score #should_check_pot temp matches 1 if items block ~ ~ ~ container.$(slot) barrier[custom_data~{pot_item:1b}] run function rotp:active/pottery_wheel/return_item {slot:$(slot)}
...
$execute if score #recipe_filled temp matches 1 unless items block ~ ~ ~ container.$(slot) light_gray_stained_glass_pane[custom_data={gui_filler:1b}] if items block ~ ~ ~ container.$(slot) * unless items block ~ ~ ~ container.$(slot) barrier[custom_data~{pottery_result:1b}] run function rotp:active/pottery_wheel/return_item {slot:$(slot)}

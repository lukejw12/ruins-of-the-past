$data modify storage rotp:temp excess_item set from block ~ ~ ~ Items[{Slot:$(slot)b}]
...
execute store result storage rotp:temp excess_item.count int 1 run scoreboard players get #excess_count temp

execute at @p[distance=..10] run summon item ~ ~ ~ {Item:{id:barrier},PickupDelay:0,Motion:[0.0,0.1,0.0]}
execute at @p[distance=..10] as @e[type=item,distance=..1,limit=1,sort=nearest] run data modify entity @s Item set from storage rotp:temp excess_item
